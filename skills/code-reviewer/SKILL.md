...
- **R3.3 修复 scope = bug scope** — 对应本技能"提交前自查"阶段的 fix commit 纯净原则；禁止"顺手重构"+ unrelated cleanup 进 fix commit
- **R5.4 PR Size 红线** — 对应本技能"PR review 流程"中的 size 评估标准（≤ 200-400 行利于 review 质量）

> **红线触发场景**：任何 AI 提交 PR / 修复 Bug / 请求审查时，必须遵循 R3.3 + R5.4；本技能提供 4 阶段 CR 工作流（自查 → 请求 → 接收 → 闭环）。
