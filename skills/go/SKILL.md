...
## Step 0: 意图识别（v2.0 整合 orch · D4.1）

> v2.0 起 orch 已合并到 go · go 内部承担 family 识别 + DAG 组装。
> 原 `skills/orch/` 已删（含 SKILL.md / references / hooks）。
...
- `references/dag-assembly.md`（rule-first 拓扑 + confidence gate + 与 Step ③ 协同）
...
/go 触发
    ...
    ▼
┌─────────────────────────────────────────────────┐
│ Step 0  意图识别（family-first · D4.1 整合 orch）│
├─────────────────────────────────────────────────┤
...
│                                                 │
│ ▸ Worker Contract v5 并发(主力 · Cursor/ZCode 同一 contract): │
│   1. 创建 feature 分支 go-<slug>                 │
│   2. 智能拆分 → 可并发子任务 DAG                  │
│   3. 每任务创建隔离 worktree + WorkerTaskPacket   │
│   4. ThreadPoolExecutor 并发 dispatch adapter     │
│      (cursor / zcode · 能力矩阵见 routing-rules)  │
│   5. 顺序 merge → merge_resolve packet 解冲突     │
│   6. 安全闸: pytest/npm test → 通过才继续         │
└─────────────────────────────────────────────────┘
    │ 所有子任务完成
    ▼
┌─────────────────────────────────────────────────┐
│ Step ⑤ 改版（v2.0 真并发模型 · D4.1）            │
├─────────────────────────────────────────────────┤
│ > v2.0 起 go 用 Agent 工具派多 subagent · 真物理 │
│   并发（替代 v1.x 的 ThreadPoolExecutor 模拟并发）│
│                                                 │
│ 用 Agent 工具同时派（多 subagent）：              │
...
│ 并发工作流：                                      │
│   go 主流程（Agent 工具同时派）                   │
│     ├─→ supervisor（subagent · 持续监控）         │
│     ├─→ loop A（worktree A · subagent）           │
│     ├─→ loop B（worktree B · subagent）           │
│     └─→ loop C（worktree C · subagent）           │
│          ↓                                       │
│   supervisor 状态文件通信（.supervisor-state.json）│
│          ↓                                       │
│   全完成（summary.done == total）或 r4_pending=true│
│          ↓                                       │
│   go 主流程 merge → G10 → 交付                   │
│                                                 │
...
9. **编排层不碰执行层**。go传最小信号,Worker Contract adapter 自行管理执行细节。
...
| 文档 | 内容 |
|------|------|
| `references/family-routing.md` | 🆕 D4.1 · 8 family 识别 + 组合白名单（吸收自 orch） |
| `references/dag-assembly.md` | 🆕 D4.1 · rule-first DAG 组装 + confidence gate（吸收自 orch） |
...
| `references/handoff-orch-schema.json` | family 阶段 handoff JSON Schema（自 orch v2 迁移） |
...
5. **Step ⑤ 并发派发**（Agent 工具多 subagent 真并发）：
   - `supervisor`（subagent · 持续监控 T1/T2/T3 状态）
   - `loop T1`（worktree `wt-orders-schema` · 立即启动）
   - `loop T2`（worktree `wt-orders-api` · 等 T1 完成）
   - `loop T3`（worktree `wt-orders-test` · 等 T2 完成）
6. **supervisor 监控时序**：
...
3. Step ①.5 → 一句话需求分析（项目=Web · 单字符串替换 · 无架构影响）
4. **委托 loop**（side-effect=single write）→ loop --level=L1
5. loop 跑 G0/G1/G9（3 核心门禁）→ 全绿
6. Step ⑧ 自动 merge（无 degraded · 门禁全绿）

**关键差异（vs 示例 1）：** L1 不拆分、不并发、不派 supervisor、不跑 G10（改动 <3 文件）。
