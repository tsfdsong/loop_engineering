# Plan Orchestrator 编排协议（v6.0 Layer 3 · 2026-06-29 重命名）

> **命名澄清**（system-review Fix #4）：
> - **Plan Orchestrator**（本文件）= skill-hub 编排**技能**（高层）
> - **Task Orchestrator** = `/go` v4.0 编排**代码任务**（工程化层，详见 `skills/go/scripts/orchestrator.py`）
>
> 两者职责清晰划分，避免命名混淆。
...
## 编排对象边界（重要 · 2026-06-29 system-review Fix #1 补充）

**Plan Orchestrator 编排的是「技能」，不是「代码任务」**：

```
Plan Orchestrator (本文件)
   ↓ 调度
   brainstorming / system-review / writing-plans / executing-plans / subagent-dd
   ↓
   这些技能产出的 plan / 报告 / 代码
   ↓
   最终由 Task Orchestrator (/go) 或 L3 loop 执行
```

**不要让 Plan Orchestrator 直接执行代码任务**。如果走完技能链后还需要实际写代码，由 `writing-plans` 的「Execution Handoff」阶段显式选择执行路径（详见 `composite-task-types.md` 第 4 类「设计+实现 路径选择」）。
