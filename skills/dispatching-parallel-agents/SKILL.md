...
description: |
  TRIGGER: 2+ 独立任务无共享状态/无顺序依赖 / 临时多问题域并行调研（前置：无现成 plan，每问题域派一个 agent，无强顺序、无逐任务双 verdict 审查——轻量汇报制）。有现成 spec-driven-development 计划 → subagent-driven-development（不用于：单任务闭环用 loop，探索性调试与共享状态任务不派发（V3 不派发条件））
