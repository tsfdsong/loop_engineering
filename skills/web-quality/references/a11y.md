...
> 本文件由 web-audit-a11y@1.0.0 迁移而来
> 入口：用户触发"无障碍 / WCAG / ARIA / 屏幕阅读器 / a11y"等关键词
> 共享前置见 `shared-setup.md`；本能力额外需 `npm i -D @axe-core/playwright`

## config 协作

`web-quality` 是 `e2e/playwright.config.ts` 的唯一 owner。
a11y 测试文件放 `e2e/tests/a11y/`，共用同一 config 的 `baseURL` / `projects`，不新建 config。
...
- 交互流程测试 → `references/regression-e2e.md`
...
axe-core 不自带 HTML 报告器。生成 HTML 报告有三种方式（实现见 `a11y/a11y-report-templates.md`）：
...
```typescript
// 方式1 示例（完整实现见 a11y/a11y-report-templates.md §方式1）
...
## references/a11y/（详细参考）

- `a11y/wcag-rules-coverage.md` — axe-core 覆盖的 WCAG 规则清单
- `a11y/antd-a11y-gotchas.md` — Antd 已知 a11y 问题 + 解决方案
- `a11y/false-positive-handling.md` — 已知误报 + 抑制规则
- `a11y/a11y-report-templates.md` — generateHtmlReport 实现 + 3 种报告生成方式
