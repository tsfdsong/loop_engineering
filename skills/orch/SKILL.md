...
5. **Single family（或白名单组合）**：默认 1 个主 family；v2.1 起允许 `dag-rules.yaml > global.allowed_combinations` 内的高频组合（如 refactor⊕test），未列入的组合仍触发澄清
...
|---|---|---|
| `refactor` ⊕ `test` | 重构后补测试 | `refactored_modules` |
| `review` ⊕ `refactor` | 审查后重构 | `review_findings` |
| `debug_fix` ⊕ `test` | 修复后加回归测试 | `fixed_modules` |
| `design_build` ⊕ `test` | 新功能带测试 | `built_modules` |
...
- ❌ 不再暴露编号式 `/orch` 用法
- ❌ 不支持自由跨 family 混编（仅允许白名单内组合，v2.1）
- ❌ 不把单技能任务强行升级成多技能编排
- ❌ 不让 LLM 自由规划任意 DAG
- ❌ 不替代 `go` / `loop` 的执行细节
