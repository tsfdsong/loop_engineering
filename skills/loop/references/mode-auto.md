...
Step ② Git 隔离（自动 · 上下文感知）
  • 检测: 当前分支是否以 "go-" 开头? 或已在 go-* worktree?
...
Step ③ 闭环编码（纯自动）
  ...
  • 门禁矩阵: 按传入/默认 L 裁剪（见 references/gate-matrix.md）
...
go 收到 failed/exhausted/stagnated → 按编排策略处理（补验收重派 / Secondary Tier / Tertiary Tier）。
