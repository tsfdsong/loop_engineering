...
name: loop
description: |
  TRIGGER: 单任务薄执行闭环（目标+验收齐全）/ gates + 自愈 / '/loop' / '闭环编码'（不用于：未定型需求用 brainstorming，多任务并行用 go，纯研究用 deep-research）
...
2. **门禁矩阵不可跳过**：本级别启用的 G0-G9 与 F1-F5，每个 ❌ 必须进自愈闭环。**G10 不在 loop 内执行**（见原则 8）
...
| **L3**（完整）| G0-G9 + F1-F5（前端验证）；**G10 由 go 跑** | 跨模块 / 架构级 / 新功能 / 含前端改动 | loop 内 G0-G9+F 全过；G10 见 go |
...
/go --level=L3 跨模块新功能       # 强制 L3（loop: G0-G9+F；G10 在 go）
/go 功能描述                       # go 评估后传入 L
```
