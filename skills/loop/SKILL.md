...
9. **合并需用户确认**（--auto 模式除外）
10. **🔴 MCP 红线（最高优先级）**：任何需要理解代码结构的操作（修改、调研、解释、分析架构、查找定义/引用），**必须先用 MCP 工具**，禁止直接 Read 全文件。标准流程：`get_repo_map` → `get_file_outline` → `search_symbols` → `Read`（仅精确行）。唯一例外：MCP 全部不可用、文件 < 50 行、需精确行内容。连续 3 次违规 = 红线事故。
