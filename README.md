...
> **loop** 闭环编码 + **go** 全自动编排 + **skill-hub** 53技能智能调度，一站式 AI 编程引擎插件。
...
### `skill-hub` — 智能路由（自动生效）
无需手动调用。收到任何任务后自动从 53 个技能中匹配最精准的一个：
...
```
loopengine/
├── skills/                  # 53 个内置技能定义（SKILL.md + references）
│   ├── loop/                # 闭环编码引擎
...
3. **自动路由** → 代理学会根据用户意图自动从 53 个技能中匹配最精准的一个
