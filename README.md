...
脚本自动检测已安装的 AI 编程工具（Claude Code / Codex / Gemini CLI / Cursor / Copilot CLI / Pi / ZCode）并执行对应安装/更新。
...
├── install.sh               # 一键安装 + 智能更新（v1.2.0 起合一，--dry-run/--force/--help）
...
详见 [install.sh Step 5](install.sh) 中的 `install_managed_rules` 函数（v1.2.2 起统一调度 7 条红线）。
