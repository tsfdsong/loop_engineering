...
> **loop** 闭环编码 + **go** 全自动编排 + **skill-hub** 45 技能智能调度（v6.1.1 合并后），一站式 AI 编程引擎插件。
...
### `skill-hub` — 智能路由（自动生效）
无需手动调用。收到任何任务后自动从 45 个技能（v6.1.1 合并后）中匹配最精准的一个：
...
## 📚 技能分类（44个内置 + 1 跨插件引用 = 45 个可用 · v6.1.1 合并后）
...
```
loopengine/
├── skills/                  # 45 个内置技能定义（v6.1.1 合并后 · SKILL.md + references）
│   ├── loop/                # 闭环编码引擎
...
3. **自动路由** → 代理学会根据用户意图自动从 45 个技能（v6.1.1 合并后）中匹配最精准的一个
