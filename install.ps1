...
# ════════════════════════════════════════════════════════════
# Step 0: Test-Version（对齐 _common.sh:78-102）
# ════════════════════════════════════════════════════════════
function Test-Version {
    ...
    if ($installed -eq $script:Version) {
        if ($Force) {
            Write-Warn "检测到 v$installed（同版）— -Force 强制重装"
        } elseif ($DryRun) {
            Write-Ok "已装 v$installed（同版）— -DryRun 将跳过安装"
        } else {
            Write-Warn "检测到已安装 v$installed（同版），5 秒后继续（强制重装请 -Force）..."
            Start-Sleep -Seconds 5
        }
    } else {
        Write-Ok "检测到 v$installed，升级到 v$($script:Version)"
    }
}
...
function Clone-Specs {
    if ($SkipSpecs) {
        Write-Info "-SkipSpecs 模式：跳过设计文档 clone"
        return
    }
    ...
    if ($DryRun) {
        Write-Info "-DryRun 模式：仅显示 clone 计划"
        Write-Info "source: $src"
        Write-Info "target: $($script:SpecsTargetDir)"
        return
    }
...
Write-Host "╚══════════════════════════════════════════════════╝" -ForegroundColor Cyan
if ($DryRun)  { Write-Info "-DryRun 模式：只检查不安装" }
if ($Force)   { Write-Info "-Force 模式：强制重装" }
if ($All)     { Write-Info "-All 模式：强制全量部署" }
Write-Host ""
...
# Step 0: 版本自检
Write-Host "🔍 Step 0: 版本自检（智能模式）..." -ForegroundColor White
Test-Version
...
if ($DryRun) {
    Write-Host ""
    Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor Cyan
    Write-Host "🔍 -DryRun 模式总结（不执行部署）" -ForegroundColor Cyan
    Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor Cyan
    $installed = if (Test-Path $script:VersionFile) { (Get-Content $script:VersionFile -Raw).Trim() } else { "" }
    Write-Info "状态: $(if($installed){'已装 v'+$installed}else{'首次安装'})"
    Write-Info "计划: $(if($Force){'强制重装'}elseif($installed -eq $script:Version){'同版，建议 -Force'}else{'升级'})"
    Write-Info "目标版本: v$($script:Version)"
    Write-Info "Agent 列表: $($script:AgentList)"
    Write-Host "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━" -ForegroundColor Cyan
    if (Test-Path $script:Work) { Remove-Item $script:Work -Recurse -Force -ErrorAction SilentlyContinue }
    exit 0
}
