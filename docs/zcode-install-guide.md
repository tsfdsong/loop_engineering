...
$dst = "$env:USERPROFILE\.zcode\cli\plugins\cache\zcode-plugins-official\loopengine\1.0.1"
mkdir -p $dst
xcopy "$src\*" "$dst\" /E /I /Y
```
...
验证：
```powershell
ls "$env:USERPROFILE\.zcode\cli\plugins\cache\zcode-plugins-official\loopengine\1.0.1\skills\" | Measure-Object | Select-Object -ExpandProperty Count
# 应输出: 53
```
...
```json
{
  "cachePath": "C:\\Users\\<你的用户名>\\.zcode\\cli\\plugins\\cache\\zcode-plugins-official\\loopengine\\1.0.1",
  "name": "loopengine",
  "source": "filesystem",
  "version": "1.0.1"
}
...
$checks = @(
    ...
    "$env:USERPROFILE\.zcode\cli\plugins\cache\zcode-plugins-official\loopengine\1.0.1\.zcode-plugin\plugin.json",
...
| | 检查缓存目录是否存在于 `cache/zcode-plugins-official/loopengine/1.0.1/` |
| | 检查 data 目录是否已创建 |
...
修复：复制缓存到 cache/zcode-plugins-official/loopengine/1.0.1/
```
...
| 缓存路径 | `cache/zcode-plugins-official/superpowers/5.1.0/` | `cache/zcode-plugins-official/loopengine/1.0.1/` |
...
> 详见 `docs/skill-loading-priority.md`（如不存在则参考本节）

### 7.6 ~/.agents/skills/ 优先路径策略（v1.0.1+）

**2026-06-29 实测发现**：ZCode 桌面版启动时**优先扫描** `~/.agents/skills/` 目录里的 `SKILL.md`，而 `marketplace.json` 中的 loopengine 注册可能被 ZCode 启动时重写丢失。一旦 `~/.agents/skills/skill-hub/SKILL.md` 残留 v5.4 旧版，CLI 缓存里的 v6.0 新版**会被遮蔽**。

**治本方案**：把 `skills/` 同步到 `~/.agents/skills/`，让 ZCode 直接从此路径加载最新技能。

#### install.sh / update.sh 自动化

v1.0.1 起，`install.sh` 和 `update.sh` 在 ZCode 桌面版分支自动执行：

1. 检测 `~/.agents/skills/skill-hub` 是否为旧版
2. 若是 → 重命名为 `~/.agents/skills/skill-hub.v5.4.backup`（一次性备份）
3. 从 CLI 缓存 `cache/zcode-plugins-official/loopengine/1.0.1/skills/` 复制全部技能到 `~/.agents/skills/`

#### 手动验证

```bash
# 应该看到 version: "6.0"（不是 5.4）
head -10 ~/.agents/skills/skill-hub/SKILL.md
```

#### 手动修复（旧版用户）

如果你的 `~/.agents/skills/skill-hub` 仍是 v5.4：

```bash
# 1) 备份
mv ~/.agents/skills/skill-hub ~/.agents/skills/skill-hub.v5.4.backup

# 2) 复制新版
cp -r ~/.zcode/cli/plugins/cache/zcode-plugins-official/loopengine/1.0.1/skills/* \
      ~/.agents/skills/

# 3) 重启 ZCode
```

#### 为什么不让 ZCode 继续走 CLI 缓存？

- ❌ ZCode 启动会重写 `marketplace.json`，loopengine 注册会反复丢失
- ❌ ZCode 优先从 `~/.agents/skills/` 扫描，回退到 CLI 缓存的概率低
- ❌ `enabledPlugins` 字段对路径扫描无影响
- ✅ `~/.agents/skills/` 简单直接，不依赖 marketplace 注册
- ✅ 与 Codex、Claude Code 等其他 AI 工具共用同一份技能
