...
2. `/go` → loop 无二次需求分析、无 writing-plans。  
...
| loop 默认 | 执行层 | Step ① 需求确认 + ② 复杂度/轮次 + ③ writing-plans |
...
| D3 | 编排与拆分 | **仅 go**（及上游 plan）；loop 不做 writing-plans |
...
| writing-plans 拆分 | 有 | **删除**；最多内部 1–3 条改动要点（不落计划文件、不问用户） |
| Git 隔离 | 有 | **保留** |
| 门禁 / 自愈 / 验证官 / 交付 | 有 | **保留**（核心） |

### 4.2 `/loop --auto` 与 go 子任务

- **禁止**产品需求确认、**禁止** writing-plans（含「自动拆分」）。  
...
1. SKILL/using-loopengine 中无「loop 主路径含需求确认 + writing-plans」。  
